...
int insert_meshpoint_hidden(char *s, float en, float structureEnergy, meshpoint_list *list, int maxEntries, int with_struct_en){
  if(list->count >= maxEntries){
    ...
    if(en > list->worstEnergy) return 0;
    else if(en == list->worstEnergy){
      if(with_struct_en){
        if(structureEnergy >= list->worstStructureEnergy) return 0;
        /* however in case we also take structural energy into account and ('en == list->worstEnergy' && 'structureEnergy < list->worstStructureEnergy') we insert this meshpoint */
          ...
          for(cur = list->first; cur != NULL; prev=cur, cur=cur->next){
            if(cur->en != en) continue;
            else if(cur->struct_en <= structureEnergy) continue;
      ...
      for(cur = list->first; cur != NULL; prev = cur,cur=cur->next){
        ...
        */
        if(cur->en < en) continue;
        else if(cur->en == en && !with_struct_en) continue;
        else if(cur->en == en && (cur->struct_en <= structureEnergy)) continue;
        else{
          ...
          cur->s = strdup(s);
          cur->en = en;
          if(with_struct_en) cur->struct_en = structureEnergy;
          cur->next = tmp;
          break;
        }
      }
      ...
      */
      ...
      prev->next = NULL;
      list->worstEnergy = prev->en;
      if(with_struct_en) list->worstStructureEnergy = prev->struct_en;
    }
  }
  else{
    ...
    */
    ...
    if(cur == NULL){
      ...
      list->worstEnergy = en;
      if(with_struct_en) list->worstStructureEnergy = structureEnergy;
    }
...
int insert_meshpoint(char *s, float en, meshpoint_list *list, int maxEntries){
  return insert_meshpoint_hidden(s, en, 0., list, maxEntries, 0);
}

int insert_meshpoint_with_struct_energy(char *s, float en, float structEn, meshpoint_list *list, int maxEntries){
...
int insert_structure_in_queue(structure_queue *queue, char *s, float en, neighbor *neighbor_list, int neighbor_cnt, float bestNeighborEn, int bestNeighborIdx, int maxStructs){
  ...
  if(queue->count < maxStructs){
    ...
    */
    if(is_in_queue(tmp->s, queue) == NULL) free(tmp->neighbor_list);
    free(tmp->s);
    free(tmp);    
...
meshpoint *is_in_queue(char *s, structure_queue *queue){
  ...
  for(cur = queue->first; cur != NULL; cur = cur->next){
    if(!strcmp(s, cur->s)) return cur;  
  }
  return NULL;
}

int sort_neighbors_by_energy_asc(const void *p1, const void *p2){
  if(((neighbor *)p1)->en > ((neighbor *)p2)->en) return 1;
  else if(((neighbor *)p1)->en < ((neighbor *)p2)->en) return -1;
  return 0;
}
