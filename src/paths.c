 ...
 */
void levelSaddlePoint(const char *seq, const char *s1, const char *s2, int maxIterations, int maxKeep, int method,
    int maxStorage) {
  ...
  while(iterator > 0){
    ...
    fprintf(stdout, "\rsearching for alternative paths...(%2.1f %% done)",
        (float) (maxIterations - iterator) / (float) maxIterations * 100.);
    fflush(stdout);
  }
  ...
  if(bestMeshPoints.count > 0){
    ...
    if(iteration < maxIterations){
      ...
      for(cur = bestMeshPoints.first; cur != NULL; cur = cur->next){
        t_path_left = levelSaddlePoint2(seq, s1, cur->s, /*&t_steps1,*/iteration + 1, maxIterations, maxKeep,
            maxStorage, maximum_distance1, maximum_distance2);
        newLeftSaddle = getSaddlePoint(t_path_left/*, t_steps1*/);

        t_path_right = levelSaddlePoint2(seq, cur->s, s2, /*&t_steps2,*/iteration + 1, maxIterations, maxKeep,
            maxStorage, maximum_distance1, maximum_distance2);
